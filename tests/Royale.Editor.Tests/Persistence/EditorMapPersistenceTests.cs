...
public sealed class EditorMapPersistenceTests
{
    ...
    public void SaveIsCanonicalReloadableAndDoesNotSerializeEditorIds()
    {
        ...
        string path = Path.Combine(directory, "graybox.json");
        EditorMapDocument document = EditorMapPersistence.Load(SourceMap);
        document.Execute(new SetMapNameCommand(document.Map.Name, "Edited"));
        EditorMapPersistence.Save(document, path, false);
        ...
        Assert.DoesNotContain("editorId", json, StringComparison.OrdinalIgnoreCase);
        Assert.False(document.IsDirty);
        Assert.Equal("Edited", MapCatalog.LoadFile(path).Name);
    ...
    public void EnforcesSaveAsFilename(string filename)
    {
        EditorMapDocument document = EditorMapPersistence.Load(SourceMap);
        Assert.NotNull(EditorMapPersistence.ValidateDestination(document, filename));
    }
